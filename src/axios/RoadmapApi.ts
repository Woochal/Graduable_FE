...
export const editSemesterRoadmapAPI = async (googleId: string, semester: number, courseIdList: number[]) => {
    try {
        console.log('Making edit request to:', `/road-map/${googleId}`);
        const response = await axiosDefault.patch(`/road-map/${googleId}`,{
            googleId: googleId,
            semester: semester,
