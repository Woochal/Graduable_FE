...
const SelectedCourseList = ({
    ...
    selectedCourseList: CourseDataAllType[];
    handleDeleteCourse: (course: CourseDataAllType) => void;
}) => {
            ...
            {selectedCourseList?.length > 0 ? (
                <S.SelectedCourseList>
                    ...
                    <S.SelectedCourseListSaveButtonDiv onClick={() => setShowSemesterModal(true)}>
                        <S.SelectedCourseListSaveButton>저장하기</S.SelectedCourseListSaveButton>
                    </S.SelectedCourseListSaveButtonDiv>
                </S.SelectedCourseList>
            ) : (
                <S.SelectedCourseListEmpty>왼쪽에서 과목을 검색한 후 추가해주세요.</S.SelectedCourseListEmpty>
            )}
        </S.SelectedCourseListContainer>
    );
};
